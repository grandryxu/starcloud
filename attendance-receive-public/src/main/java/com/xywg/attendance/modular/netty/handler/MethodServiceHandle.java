...
import static com.xywg.attendance.common.global.RequestUrlEnum.HANDLE_GET_ALL_COMMAND;
...
public class MethodServiceHandle {
    ...
    void handleGetCommand(ChannelHandlerContext ctx, TransmissionMessageTemplate template) {
        ...
        String sn = map.get("SN");
        // 判断是否有心跳信息，没有说明掉线或是新上线，下发所有人员
        if(redisUtil.get(DEVICE_INFO_STATUS_REDIS_KEY + sn) == null){
            // 请求所有人员
            template.setUrl(template.getUrl().replaceAll("getrequest","getAllRequest"));
            template.setMethod(template.getMethod().replaceAll("handleGetCommand","handleGetAllCommand"));
        ...
        if (redisCommand == null) {
            ...
            if (storeToRedis.size() > 0) {
                ...
                redisUtil.set(DEVICE_GET_COMMAND_REDIS_KEY + sn, storeToRedis,60*24L);
            } else {
