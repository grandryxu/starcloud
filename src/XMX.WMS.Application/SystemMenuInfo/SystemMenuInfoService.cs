...
{
    [AbpAuthorize(PermissionNames.Pages_Roles,PermissionNames.Pages_Users)]
    public class SystemMenuInfoService : AsyncCrudAppService<SystemMenuInfo, SystemMenuInfoDto, Guid, SystemMenuInfoPagedRequest, SystemMenuInfoCreatedDto, SystemMenuInfoUpdatedDto>, ISystemMenuInfoService
    {
        public SystemMenuInfoService(IRepository<SystemMenuInfo, Guid> repository) : base(repository)
        {
        }
        ...
        protected override IQueryable<SystemMenuInfo> CreateFilteredQuery(SystemMenuInfoPagedRequest input)
        {
        ...
        public List<SystemMenuInfoTreeList> GetSystemMenuInfoTree(SystemMenuInfoPagedRequest input)
        {
            //父级
            ...
            var relist = new List<SystemMenuInfoTreeList>();
            foreach (var item in plist)
            {
                var arr = list.FindAll(x => x.menu_parent_id == item.Id).ToList();
                List<SystemMenuInfoNode> output = new List<SystemMenuInfoNode>();
                foreach (var i in arr)
                    output.Add(new SystemMenuInfoNode { id = i.Id, label = i.menu_function_name });
                relist.Add(new SystemMenuInfoTreeList { id = item.Id, label = item.menu_function_name, children = output });
            }
            return relist;
        }
        ...
        public List<SystemMenuInfoDto> GetSystemMenuInfoSelectedList()
        {
            ...
            var relist = new List<SystemMenuInfoDto>();
            foreach (var item in list)
                relist.Add(new SystemMenuInfoDto
                {
                    Id = item.Id,
        ...
        /// <returns></returns>
        public async Task<List<SystemMenuInfo>> GetSystemSubMenuInfoList(SubMenuRequest subMenuRequest)
        {
