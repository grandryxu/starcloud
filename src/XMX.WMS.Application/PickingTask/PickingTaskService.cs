...
namespace XMX.WMS.PickingTask
{
    ...
    /// 版 本：
    /// 创建人：Haoran
    /// 日 期：2020-05-13 16:51:49
    /// 描 述：
    ///</summary>
    public class PickingTaskService : AsyncCrudAppService<PickingTask, PickingTaskDto, Guid, PickingTaskPagedRequest, PickingTaskCreatedDto, PickingTaskUpdatedDto>, IPickingTaskService
    {
        ...
        public PickingTaskService(IRepository<PickingTask, Guid> repository,
        ...
        protected override IQueryable<PickingTask> CreateFilteredQuery(PickingTaskPagedRequest input)
        {
            //, x => x.Slot1.Row.Warehouse
            return Repository.GetAllIncluding(x => x.Port1, x => x.Slot1, x => x.Slot1.Warehouse)
                    .WhereIf(AbpSession.UserId != 1, x => x.picking_company_id == UserCompanyId)
                    .WhereIf(input.picking_mode != null, x => x.picking_mode == input.picking_mode)
                    .WhereIf(input.picking_execute_flag != TaskExecuteFlag.非完成, x => x.picking_execute_flag == input.picking_execute_flag)
                    .WhereIf(input.picking_execute_flag == TaskExecuteFlag.非完成, x => x.picking_execute_flag != TaskExecuteFlag.已完成)
                    .WhereIf(input.picking_manual_flag != null, x => x.picking_manual_flag == input.picking_manual_flag)
                    .WhereIf(input.picking_creat_datetime != null, x => x.CreationTime == input.picking_creat_datetime)
                    .OrderBy(x => x.picking_execute_flag);
        }
        ...
        public int GetNowTaskNum()
        {
            return Repository.GetAllIncluding(x => x.Slot1.Row.Warehouse)
                                        ...
                                        .Where(x => x.picking_execute_flag != TaskExecuteFlag.已完成).Count();
        }
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public override Task<PickingTaskDto> Get(EntityDto<Guid> input)
        {
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public override async Task<PickingTaskDto> Create(PickingTaskCreatedDto input)
        {
            var flag = Repository.GetAll().Where(x => x.picking_no == input.picking_no).Any();
            if (flag)
            {
                EncodingRule.EncodingRuleService er = new EncodingRule.EncodingRuleService(_erRepository, _taskRepository);
                input.picking_no = er.GetEncodingRule("TaskCode");
            }
            input.picking_company_id = UserCompanyId;
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public override async Task<PickingTaskDto> Update(PickingTaskUpdatedDto input)
        {
            var res = await base.Update(input);
            SlotInfo.SlotInfo slot = _sRepository.FirstOrDefault(x => x.Id == input.picking_slot_code);
            switch (input.picking_execute_flag)
            {
                case TaskExecuteFlag.输送机:
                    if ((input.picking_mode == TaskType.出库 || input.picking_mode == TaskType.空托盘出库) && input.picking_manual_flag == TaskManualFlag.自动)//目前没有考虑双伸
                    {
                ...
                case TaskExecuteFlag.已完成:
                    if ((input.picking_mode == TaskType.入库 || input.picking_mode == TaskType.空托盘入库) && input.picking_manual_flag == TaskManualFlag.自动)
                    {
        ...
        {
            // 查询任务
            PickingTask task = Repository.FirstOrDefault(x => x.Id == taskid && x.picking_execute_flag == TaskExecuteFlag.已完成 && x.IsDeleted == false);
            switch (task.picking_mode)
            {
                case TaskType.入库:
                    Taskimp(task.Id);
                    break;
        ...
        private void Taskmove(PickingTask task)
        {
            InventoryInfo.InventoryInfo inventory = _iRepository.FirstOrDefault(x => x.inventory_stock_code == task.picking_stock_code && x.inventory_status == InventoryStatus.出库 && x.inventory_slot_code == task.picking_slot_code);
            if (inventory != null)
            {
                //有库存信息，更新库存信息
                inventory.inventory_slot_code = Guid.Parse(task.picking_inslot_code.ToString());
                inventory.inventory_status = InventoryStatus.可用;
                _iRepository.Update(inventory);
            }
            SlotInfo.SlotInfo slot = _sRepository.FirstOrDefault(x => x.Id == task.picking_inslot_code);
            slot.slot_stock_status = SlotStock.有库存;
            _sRepository.Update(slot);
        }
        ...
        public bool CreateAutomaticTask(List<Guid> idList)
        {
            ...
            foreach (ExportBillhead.ExportBillhead head in headList)
            {
                ...
                foreach (ExportBillbody.ExportBillbody body in bodyList)
                {
                    ...
                    {
                        // 查找是否有同托盘出库任务
                        PickingTask task = Repository.FirstOrDefault(x => x.picking_stock_code == inventory.inventory_stock_code && x.picking_mode == TaskType.出库 && x.picking_execute_flag != TaskExecuteFlag.已完成);
                        if (task != null)
                        {
            ...
            foreach (ExportBillbody.ExportBillbody body in bodyList)
            {
                ...
                foreach (ExportOrder.Dto.ExportOrderUpdatedDto orderDto in input.orderList)
                {
                    ...
                    if (inventory == null)
                        throw new UserFriendlyException("无可用库存！");
                    // 查找是否有同托盘出库任务
                    PickingTask task = Repository.FirstOrDefault(x => x.picking_stock_code == orderDto.exporder_stock_code && x.picking_mode == TaskType.出库 && x.picking_execute_flag != TaskExecuteFlag.已完成);
                    if (task == null)
                    {
                        task = new PickingTask();
                        EncodingRule.EncodingRuleService er = new EncodingRule.EncodingRuleService(_erRepository, _taskRepository);
                        string picking_no = er.GetEncodingRule("TaskCode");
                        // 任务号5位
                        task.picking_no = picking_no;
                        // 优先级
                        task.picking_priority = 5;
                        // 任务方式(1入库；2出库；3移库；4口对口)
                        task.picking_mode = TaskType.出库;
                        // 库位
                        task.picking_slot_code = orderDto.exporder_slot_code.Value;
                        // 托盘码
                        task.picking_stock_code = orderDto.exporder_stock_code;
                        // 手自标志(1自动；2手动)                                                                                                                                               
                        task.picking_manual_flag = TaskManualFlag.自动;
                        task.picking_execute_flag = TaskExecuteFlag.待执行;
                        task.picking_company_id = head.exphead_company_id;
        ...
        /// <returns></returns>
        private PickingTask CreateTask(string taskType, string id, string slotcode, string stockcode, string warehouseId, string goodsId)
        {
            ...
            EncodingRule.EncodingRuleService er = new EncodingRule.EncodingRuleService(_erRepository, _taskRepository);
            // 任务号5位
            task.picking_no = er.GetEncodingRule("TaskCode");
            // 优先级
            task.picking_priority = 5;
            // 库位
            task.picking_slot_code = Guid.Parse(slotcode);
            // 手自标志(1自动；2手动)
            task.picking_manual_flag = TaskManualFlag.自动;
            task.picking_execute_flag = TaskExecuteFlag.待执行;
            task.picking_company_id = UserCompanyId;
            switch (taskType)
            {
                case "ImpstockTask":
                    ImportStock.ImportStock importStock = _isRepository.Single(x => x.Id == Guid.Parse(id));
                    task.picking_mode = TaskType.空托盘入库;
                    task.picking_stock_code = importStock.impstock_stock_code;// 托盘码
                    task = Repository.Insert(task);
                ...
                case "ImporderTask":
                    ...
                    task.picking_mode = TaskType.入库;
                    task.picking_stock_code = importOrder.imporder_stock_code;// 托盘码
                    task = Repository.Insert(task);
                    ...
                    break;
                case "ExpstockTask":
                    task.picking_mode = TaskType.空托盘出库;
                    task.picking_stock_code = stockcode;
                    task = Repository.Insert(task);
                    break;
                case "ExporderTask":
                    task.picking_mode = TaskType.出库;
        ...
        public bool CreateExportTask(List<string> codeList)
        {
            ...
            {
                // 查找是否有同托盘出库任务
                PickingTask task = Repository.FirstOrDefault(x => x.picking_stock_code == inventory.inventory_stock_code && x.picking_mode == TaskType.空托盘出库 && x.picking_execute_flag != TaskExecuteFlag.已完成);
                if (task == null)
                {
                    PickingTask PickingTask = CreateTask("ExpstockTask", "", inventory.inventory_slot_code.ToString(), inventory.inventory_stock_code, null, null);
                    // 库存状态修改
                    inventory.inventory_status = InventoryStatus.出库;
                    _iRepository.Update(inventory);
                    ...
                    stock.expstock_task_id = PickingTask.Id;
                    stock.expstock_company_id = task.picking_company_id;
