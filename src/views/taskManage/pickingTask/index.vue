    ...
    <div class="iot-list" ref="page">
                ...
                <el-form inline label-width="100px" :model="searchForm">
                    <div class="iot-form-row">
                        <el-form-item label="状态">
                            <el-select class="iot-w200" v-model="searchForm.picking_execute_flag" placeholder="请选择"
                                       clearable>
                        ...
                        <el-form-item label="类型">
                            <el-select class="iot-w200" v-model="searchForm.picking_mode" placeholder="请选择" clearable>
                                <el-option v-for="item in pickingmodeList" :key="item.id" :label="item.modeName"
                                           :value="item.id"></el-option>
                            </el-select>
                        </el-form-item>
                        ...
                        <el-form-item label="来源">
                            <el-select class="iot-w200" v-model="searchForm.picking_manual_flag" placeholder="请选择"
                                       clearable>
        ...
        <div class="iot-table">
            <el-table :data="tableData" stripe style="width: 100%" border @selection-change="handleSelectionChange"
                ...
                <el-table-column align="center" prop="picking_no" label="任务号" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="picking_priority" label="优先级" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="picking_mode" label="类型" min-width="100" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span v-if="scope.row.picking_mode === 1">入库</span>
                        <span v-else-if="scope.row.picking_mode === 2">出库</span>
                        <span v-else-if="scope.row.picking_mode === 3">移库</span>
                        <span v-else-if="scope.row.picking_mode === 4">口对口</span>
                        <span v-else-if="scope.row.picking_mode === 5">回流</span>
                        <span v-else-if="scope.row.picking_mode === 6">空托盘入库</span>
                        <span v-else-if="scope.row.picking_mode === 7">空托盘出库</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column align="center" prop="slot1.slot_code" label="库位编码" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="picking_stock_code" label="托盘" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <!-- <el-table-column align="center" prop="material_name" label="物料名称" min-width="100"
                 ...
                 <el-table-column align="center" prop="exporder_quantity" label="数量" min-width="100"
                                  show-overflow-tooltip></el-table-column>-->
                <el-table-column align="center" prop="picking_execute_flag" label="执行状态" min-width="100"
                                 show-overflow-tooltip :formatter="executeTransform"></el-table-column>
                <el-table-column align="center" prop="picking_creat_datetime" label="下发时间" min-width="100"
        ...
        <el-dialog class="iot-dialog" title="修改" :visible.sync="singleDialogVisible" width="488px"
            ...
            <el-form inline ref="areaForm" :model="ruleForm">
                <div class="iot-form-row">
                    <el-form-item label="状态" label-width="100px" prop="picking_mode">
                        <el-select class="iot-w240" v-model="ruleForm.picking_execute_flag" placeholder="请选择" clearable>
                            <el-option v-for="item in statusList" :key="item.id" :label="item.modeName"
                                       :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>

                    <el-form-item label="优先级" label-width="100px" prop="picking_mode">
                        <el-select class="iot-w240" v-model="ruleForm.picking_priority" placeholder="请选择" clearable>
            ...
            <el-form inline ref="dialogForm" :model="dialogForm">
                <div class="iot-form-row">
                    <el-form-item label="托盘号" label-width="100px" prop="material_code">
                        <el-input class="iot-w240" placeholder="请输入托盘号" maxlength="50"
                                  v-model="dialogForm.picking_stock_code" :disabled="true" clearable></el-input>
                    </el-form-item>
                ...
                <div class="iot-form-row">
                    <el-form-item label="任务号" label-width="100px" prop="number">
                        <el-input class="iot-w240" placeholder="请输入任务号" maxlength="50" v-model="dialogForm.picking_no"
                                  :disabled="true" clearable></el-input>
                    </el-form-item>
                    <el-form-item label="任务状态" label-width="100px" prop="danwei">
                        <el-input class="iot-w240" placeholder="请输入任务状态" maxlength="50"
                                  v-model="dialogForm.picking_execute_flag" :disabled="true" clearable></el-input>
                    </el-form-item>
                </div>
                <div class="iot-form-row">
                    <div style="text-align: center;font-size: 30px;padding: 10px 0px;">
                        <template>
                            <span v-if="picking_mode === 1">入库流水</span>
                            <span v-else-if="picking_mode === 2">出库流水</span>
                            <span v-else-if="picking_mode === 3">移库流水</span>
                            <span v-else-if="picking_mode === 4">口对口流水</span>
                            <span v-else-if="picking_mode === 5">回流流水</span>
                            <span v-else-if="picking_mode === 6">空托盘入库流水</span>
                            <span v-else-if="picking_mode === 7">空托盘出库流水</span>
                        </template>
        ...
        <el-dialog class="iot-dialog" :title="dialogTitleA" :visible.sync="dialogVisibleA" width="844px" append-to-body>
            <el-form inline ref="dialogFormA" :model="ManualForm" :rules="ManualFormrules">
                <div class="iot-form-row">
                    <el-form-item label="任务号" label-width="120px" prop="picking_no">
                        <el-input class="iot-w240" v-model="ManualForm.picking_no" placeholder="请输入任务号" maxlength="30"
                                  clearable :disabled="true"></el-input>
                    </el-form-item>
                    <el-form-item label="优先级" label-width="120px" prop="picking_priority">
                        <el-select class="iot-w240" v-model="ManualForm.picking_priority" placeholder="请选择" clearable>
                            <el-option v-for="item in sortList" :key="item.id" :label="item.id"
                                       :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="类型" label-width="120px" prop="picking_mode">
                        <el-select class="iot-w240" v-model="ManualForm.picking_mode" placeholder="请选择" clearable
                                   @change="getpickingmode">
                            <el-option v-for="item in pickingmodeList" :key="item.id" :label="item.modeName"
                                       :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="库位编码" label-width="120px" prop="picking_slot_code">
                        <el-input class="iot-w240" v-model="ManualForm.picking_slot_code" placeholder="请输入库位编码"
                                  maxlength="30" clearable></el-input>
                    </el-form-item>

                    <el-form-item v-if="ManualForm.picking_mode=='3'" label="移入库位编码" label-width="120px"
                                  prop="picking_inslot_code">
                        <el-input class="iot-w240" v-model="ManualForm.picking_inslot_code" placeholder="请输入移入库位编码"
                                  maxlength="30" clearable></el-input>
                    </el-form-item>


                    <el-form-item label="托盘" label-width="120px" prop="picking_stock_code">
                        <el-input class="iot-w240" v-model="ManualForm.picking_stock_code" placeholder="请输入托盘编码"
    ...
    export default {
        ...
        methods: {
            ...
            async GetEncodingRuleCode() {
                ...
                this.ManualForm.picking_no = code;
            },
            //查询该仓库下的库位是否存在
            async checkExsit() {
                let params = {
                    slot_code: this.ManualForm.picking_slot_code,
                ...
                if (res) {
                    ...
                    if (num == "0") {
                        ...
                        if (this.ManualForm.picking_inslot_code && this.ManualForm.picking_mode == '3') {
                            let params = {
                                slot_code: this.ManualForm.picking_inslot_code,
                                slot_warehouse_id: this.ManualForm.warehouse_id
                            };
                            let res = await GetSlotCodeIdApi(params);
                            if (res) {
                                ...
                                if (num == "0") {
                                    ...
                                    this.$msgbox({
                                        ...
                                        params.picking_inslot_code=result1;
                                        params.picking_slot_code=result;
                                        params.picking_execute_flag = 1;
                                        params.picking_manual_flag=2;
            ...
            click_Manualsubmit() {
                this.$refs["dialogFormA"].validate(valid => {
                    ...
                    if (this.ManualForm.picking_slot_code) {
                        this.checkExsit();
                    } else {
            ...
            getpickingmode(val) {
                if (val === 3) {
                    this.ManualFormrules.picking_stock_code = null;
            ...
            async GetAll() {
                let params = {
                 ...
                 if(params.picking_execute_flag==null||params.picking_execute_flag=="")
                {
                params.picking_execute_flag=10;
                }
            ...
            showdialog() {
                if (this.multipleSelection.length == 0) {
                    ...
                    if(this.multipleSelection[0].picking_manual_flag=='1')
                    {
                        this.dialogTitle = '查看'
                        this.dialogVisible = true;
                        let row = this.multipleSelection[0];
                        this.dialogForm.picking_stock_code = row.picking_stock_code;
                        this.dialogForm.slot_code = row.slot1.slot_code;
                        this.dialogForm.picking_no = row.picking_no;
                        this.statusList.forEach(ele => {
                            if (row.picking_execute_flag === ele.id) {
                                this.dialogForm.picking_execute_flag = ele.modeName;
                            }
                        });
                        this.picking_mode = row.picking_mode;
            ...
            async Getlist() {
                ...
                if (this.picking_mode == '1') {

                } else if (this.picking_mode == '2') {
                } else if (this.picking_mode == '3') {
                } else if (this.picking_mode == '4') {
                } else if (this.picking_mode == '5') {
                } else if (this.picking_mode == '6') {

                    let data = await getkyprkListApi(params);
                    if (data) {
                        this.lstableData = data.items || [];
                    }
                } else if (this.picking_mode == '7') {
                }
            ...
            handleModify() {
                if (this.multipleSelection.length == 0) {
                    ...
                    this.ruleForm.picking_execute_flag = ''
                    this.ruleForm.picking_priority = ''
            ...
            empty() {
                this.ManualForm.picking_priority = ''
                this.ManualForm.picking_mode = ''
                this.ManualForm.warehouse_id = ''
                this.ManualForm.picking_slot_code = ''
                this.ManualForm.picking_inslot_code = ''
                this.ManualForm.picking_stock_code = ''
            },
            //点击执行
            click_modify(val) {
                this.$refs["areaForm"].validate(valid => {
                    ...
                    this.$msgbox({
                        ...
                        if (this.ruleForm.picking_execute_flag)
                            params.picking_execute_flag = this.ruleForm.picking_execute_flag
                        if (this.ruleForm.picking_priority)
                            params.picking_priority = this.ruleForm.picking_priority
                        params.slot1 = null;
            ...
            async Get(row) {
                ...
                let obj = {
                    id: row.id,
                    // picking_no:res.picking_no,
                    picking_priority: res.picking_priority,
                    // picking_mode:picking_mode,
                    // picking_slot_code:res.picking_slot_code,
                    // picking_stock_code:res.picking_stock_code,
                    picking_execute_flag: res.picking_execute_flag,
                    // picking_manual_flag:res.picking_manual_flag,
                    // picking_port:res.picking_port,
