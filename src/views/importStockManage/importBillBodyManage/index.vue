    ...
    <div class="iot-list" ref="page">
        <div class="iot-form">
            <div class="layout__search">
                <el-form inline label-width="100px" :model="searchForm">
                    <el-form-item label="入库单号">
                        <el-input class="iot-w200" placeholder="请输入入库单号" maxlength="50"
                                  v-model="searchForm.imphead_code" clearable></el-input>
                    </el-form-item>
                    <el-form-item label="大批号">
                        <el-input class="iot-w200" placeholder="请输入大批号" maxlength="50"
                                  v-model="searchForm.imporder_batch_no" clearable></el-input>
                    </el-form-item>
                    <el-form-item label="小批号">
                        <el-input class="iot-w200" placeholder="请输入小批号" maxlength="50"
                                  v-model="searchForm.imporder_lots_no" clearable></el-input>
                    </el-form-item>
                    <el-form-item label="客户名称">
                        <el-select class="iot-w200" v-model="searchForm.custom_name" placeholder="请选择" clearable>
                            <el-option
                                    v-for="(item,index) in customList" :key="index" :label="item.custom_name"
                                    :value="item.custom_name"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="托盘号码">
                        <el-input class="iot-w200" placeholder v-model="searchForm.imporder_stock_code"></el-input>
                    </el-form-item>
                    <el-form-item label="库位">
                        <el-input class="iot-w200" placeholder v-model="searchForm.imporder_slot_code"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="状态">
                        <el-select class="iot-w200" v-model="searchForm.imporder_execute_flag" placeholder="请选择"
                                   clearable>
                            <el-option v-for="item in executeList" :key="item.value" :label="item.label"
                                       :value="item.value"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="入库日期">
                        <el-date-picker class="iot-w150" clearable v-model="searchForm.startDate" type="date"
                                        placeholder="起始日期"></el-date-picker>
                        <span class="el-icon-minus" style="color:#ccc"></span>
                        <el-date-picker class="iot-w150" clearable v-model="searchForm.endDate" type="date"
            ...
            <div class="layout__btns">
                <div ref="permissions">
                    <!-- <el-button type="primary" plain icon="el-icon-search" @click="handleAnalogScan">模拟扫描</el-button> -->
                    <el-button type="primary" plain icon="el-icon-edit-outline" @click="handleEdit">编辑</el-button>
                    <el-button type="primary" plain icon="el-icon-delete" @click="handleInvalid">作废</el-button>
        ...
        <div class="iot-table">
            <el-table id="out-table" ref="print" :data="tableData" stripe style="width: 100%" border
                ...
                <el-table-column align="center" prop="importBillbody.importBillhead.imphead_code" label="入库单号"
                                 min-width="100" show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="goods.goods_code" label="物资编码" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <!-- :formatter="toGoodsCode" -->
                <el-table-column align="center" prop="goods.goods_name" label="物资名称" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <!-- :formatter="toGoodsName" -->
                <el-table-column align="center" prop="imporder_batch_no" label="大批号" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="imporder_lots_no" label="小批号" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="imporder_stock_code" label="托盘号码" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="slot.slot_code" label="库位" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="custom_name" label="客户名称" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="imporder_quantity" label="数量" min-width="100"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" prop="unit_name" label="单位名称" min-width="100" show-overflow-tooltip>
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" prop="imporder_execute_flag" label="执行状态" min-width="100"
                                 show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span v-if="scope.row.imporder_execute_flag === 1">未执行</span>
                        <span v-else-if="scope.row.imporder_execute_flag === 2">执行中</span>
                        <span v-else-if="scope.row.imporder_execute_flag === 3">已完成</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="imporder_noused_flag" label="是否作废" min-width="100"
                                 show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span v-if="scope.row.imporder_noused_flag === 1">正常</span>
                        <span v-else-if="scope.row.imporder_noused_flag === 2">作废</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog class="iot-dialog" :title="dialogTitle" :visible.sync="dialogVisible" width="824px" append-to-body>
            <el-form inline ref="dialogForm" :model="ruleForm" :rules="rules">
                <div class="iot-form-row">
                    <el-form-item label="物资名称" label-width="100px" prop="impbody_goods_id">
                    ...
                    <el-form-item label="物资编码" label-width="100px" prop="impbody_goods_id">
                        <el-select class="iot-w240" v-model="ruleForm.impbody_goods_id" placeholder="请选择" disabled>
                            <el-option v-for="item in goodsList" :key="item.id" :label="item.goods_code"
                                       :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                </div>
                <div class="iot-form-row">
                    <el-form-item label="批次" label-width="100px" prop="impbody_batch_no">
                        <el-input class="iot-w240" v-model="ruleForm.impbody_batch_no" maxlength="50"
                                  placeholder="请输入批次" clearable disabled></el-input>
                    </el-form-item>
                    <el-form-item label="数量" label-width="100px" prop="impbody_binding_quantity">
                        <el-input class="iot-w240" v-model="ruleForm.impbody_binding_quantity" placeholder="请输入数量"
                                  maxlength="20" clearable disabled></el-input>
                    </el-form-item>
                </div>
                <div class="iot-form-row">
                    <el-form-item label="有效期" label-width="100px" prop="impbody_vaildate_date">
                        <el-date-picker class="iot-w240" clearable v-model="ruleForm.impbody_vaildate_date" type="date"
                                        placeholder="请选择" disabled></el-date-picker>
                    </el-form-item>
                    <el-form-item label="质量状态" label-width="100px" prop="impbody_quality_status">
                        <el-select class="iot-w240" v-model="ruleForm.impbody_quality_status" placeholder="请选择"
                                   disabled>
                            <el-option v-for="item in qualityList" :key="item.id" :label="item.quality_name"
                                       :value="item.id"></el-option>
                        </el-select>
                    </el-form-item>
                </div>
                <div v-if="dialogTitle === '修改'">
                    <h1 style="text-align:center;font-size:16px;font-weight:bolder;">
                        {{$t('importBillBody.goodsPackDetail')}}</h1>
                    <div class="layout__btns">
                        <div>
                    ...
                    <div class="iot-table">
                        <el-table :data="tableScanData" stripe style="width: 100%" border @row-dblclick="handleScanEdit"
                                  @selection-change="handleSelectionChangeOrder">
                            <el-table-column type="selection" width="55"></el-table-column>
                            <el-table-column align="center" type="index" :label="$t('public.serialNumber')"
                                             width="50"></el-table-column>
                            <el-table-column align="center" prop="imporder_box_code"
                                             :label="$t('importBillBody.packCode')" min-width="100"
                                             show-overflow-tooltip></el-table-column>
                            <el-table-column align="center" prop="imporder_quantity" :label="$t('public.quantity')"
                                             min-width="100" show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <span v-show="!scope.row.isCanEdit">{{scope.row.imporder_quantity}}</span>
                                    <el-input v-show="scope.row.isCanEdit" v-model="scope.row.imporder_quantity"
                                              clearable></el-input>
                                </template>
                            </el-table-column>
                            <!-- <el-table-column
                ...
                <div v-else-if="dialogTitle === '模拟扫描'">
                    <div class="layout__btns">
                        <div>
                            ...
                            <el-button type="primary" plain icon="el-icon-search" @click="handleDeleteScan">删除
                            </el-button>
                        </div>
                    </div>
                    <div class="iot-table">
                        <el-table
                                ...
                                stripe style="width: 100%" border @selection-change="handleSelectionScanChange">
                            <el-table-column type="selection" width="55"></el-table-column>
                            ...
                            <el-table-column align="center" prop="imporder_stock_code" label="托盘码" min-width="100"
                                             show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-input class="iot-w150" v-model="scope.row.imporder_stock_code"
                                              clearable></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column align="center" prop="imporder_box_code" label="箱码" min-width="100"
                                             show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-input class="iot-w150" v-model="scope.row.imporder_box_code"
                                              clearable></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column align="center" prop="imporder_quantity" label="数量" min-width="100"
                                             show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <el-input class="iot-w150" v-model="scope.row.imporder_quantity"
                                              clearable></el-input>
                                </template>
                            </el-table-column>
                        </el-table>
                    ...
                    <div class="iot-pagination">
                        <el-pagination background layout="prev, pager, next" :total="tableScanData.length"
                                       :page-size="scanPageSize" :current-page.sync="currentScanPage"
                                       @current-change="handleCurrentChangeScan"></el-pagination>
                    </div>
                </div>
            </el-form>
    ...
    export default {
        ...
        methods: {
            ...
            handleScan() {
                ...
                if (data.length === 0) {
                    data.push({
                        ...
                        imporder_quality_status: this.ruleForm.impbody_quality_status,
                    })
                } else {
                    data.push({
            ...
            async handleMaintask() {
                if (this.multipleSelection.length == 1) {
                    if (this.multipleSelection[0].imporder_execute_flag == '1')

                        this.CreateMainTask(this.multipleSelection[0]);
                    else
                        this.$message.error('只有未执行的流水才能生成任务呀！');
                } else {
                    //this.text_selectOne  全局定义的提示  在textConfig.js中
                    this.$message.error(this.text_selectOne);
                }
            ...
            CreateMainTask(rows) {
                ...
                this.$msgbox({
                    ...
                    let param = {
                        imporder_stock_code: rows.imporder_stock_code
                    }
                    let res = await addMainTaskApi(param);
                    if (res) {
                        this.$message({
                            type: "success",
                            message: "立库成功"
