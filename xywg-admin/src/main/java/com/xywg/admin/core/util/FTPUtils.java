...
public class FTPUtils {
    ...
    private static FTPClient connect() {
        ...
        try {
            //连接FTP服务器
            ftpClient.connect(environment.getProperty("ftp.host"), Integer.parseInt(environment.getProperty("ftp.port")));
            //登录FTP服务器
            ftpClient.login(environment.getProperty("ftp.username"), environment.getProperty("ftp.password"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return ftpClient;
...
//            inputStream.close();
    ...
    public static void main(String[] args) {
        try {
            ...
            ftpClient.setControlEncoding("UTF-8");
            //连接FTP服务器
            ftpClient.connect("192.168.10.124", 21);
            //登录FTP服务器
            ftpClient.login("jack", "qwer1234");
            if(ftpClient.changeWorkingDirectory("2019/6/4/face")) {
               FTPFile[] files = ftpClient.listFiles("c336da88-2b6d-4d24-89fb-9dd4320faad0.png");
               if(null != files && files.length > 0) {
                   FTPFile file = files[0];
                   InputStream is = ftpClient.retrieveFileStream("c336da88-2b6d-4d24-89fb-9dd4320faad0.png");
                   //图片压缩的demo ImageUtils.compressImage(is, file.getSize());
                   //图片加水印的demo
                   ImageUtils.addWatermark(is, "2019-01-01 12:11:11", null,0.7f);
               }
            }
        } catch (IOException e) {
            e.printStackTrace();
