package com.xywg.admin.oauth.web;
...
public abstract class WebUtils {
    ...
    public static void writeOAuthJSONObjectResponse(HttpServletResponse response, JSONObject oAuthResponse) {

        final int responseStatus = 200;
        try {

            final Map<String, String> headers = new HashedMap();
            for (String key : headers.keySet()) {
                response.addHeader(key, headers.get(key));
            }
            // CORS setting
            ...
            final PrintWriter out = response.getWriter();
            out.print(oAuthResponse.toString());
            out.flush();
        } catch (IOException e) {
            throw new IllegalStateException("Write OAuthResponse error", e);
        }
