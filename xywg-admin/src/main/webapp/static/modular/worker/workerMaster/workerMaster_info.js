    ...
    $.ajax({
            ...
            success: function (data) {
                if (data.length == 1) {
                    ...
                    $('#flatType').attr('disabled', false);

                    $("input").attr("disabled", true);
                    $("select").attr("disabled", true);
                    $("#headImge").val(data[0].headImage);
                    $("#faceImge").val(data[0].face);
                    $("#isFaceImage").val(data[0].isFace);
                    $("#faceImg").attr('src',data[0].headImage != ''?'/labor/'+data[0].face:'/static/img/avatar2.png');
                    $("#fileImg").attr('src',data[0].face != ''?'/labor/'+data[0].headImage:'/static/img/avatar1.png');
               ...
               // $('#workTypeCode').chosen({search_contains: true, no_results_text: "暂无结果"});
                ...
                $('#idCardType').attr('disabled', false);
                $('#idCardNumber').attr('disabled', false);
                $('#workerName').attr('disabled', false);
...
//    }else{
    ...
    if(cardNumber != null && cardNumber != ''){
        ...
        if(($("#isEnterprise").val() == 0)&&(!WorkerMasterInfoDlg.workerMasterInfoData.projectCode || !WorkerMasterInfoDlg.workerMasterInfoData.teamSysNo)){
            Feng.info("请选择项目和班组");
            return;
        }
  
    //添加图片
    if($('#headImge').val() && $('#headImge').val() != '') {
        this.workerMasterInfoData.headImage = $('#headImge').val();
        this.workerMasterInfoData.isFace = $('#isFaceImage').val();
    }
    
    //添加身份证图片
    if($('#faceImge').val() && $('#faceImge').val() != '') {
        this.workerMasterInfoData.face = $('#faceImge').val();
    }
    ...
    this.workerMasterInfoData.isNeedCheck = $("#isNeedCheck").val();
    //提交信息
...
//        WorkerMasterInfoDlg.workerMasterInfoData.teamSysNo = $("#teamLeader").val();
//    }else{
        WorkerMasterInfoDlg.workerMasterInfoData.teamSysNo = $("#teamSysNoValue").val();
    }
...
/**
*
* @param selected 班组change
*/
WorkerMasterInfoDlg.teamChange = function () {
	$("#teamSysNoValue").val($("#team").val());
}
...
WorkerMasterInfoDlg.uploadHeadImg = function(){
...
function fileUpload() {
    ...
    var files = $('#headImgFile')[0].files;
    if(files.length === 0){
        return;
    }
	var _name = $("#headImgFile").val();
	_fileName = _name.substring(_name.lastIndexOf(".") + 1).toLowerCase();
	if (_fileName !== "png" && _fileName !== "jpg") {
        Feng.error("上传图片格式不正确，请重新上传");
        return;
    }
//	alert(_name);
//	$("#faceImg").attr('src', _name);
    ...
    $.ajax({
        ...
        success: function (data) {
           ...
           // Feng.success(data);
            $('#headImgFile').val("");
            $('#faceImge').val(data.path);
            $("#faceImg").attr('src',data.path != ''?'/labor/'+data.path:'/static/img/avatar2.png');
        	}
        	},
        error: function (data) {
