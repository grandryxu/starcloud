...
ProjectMasterInfoDlg.addSubmit = function () {
    ...
    var num=0;
   var projectManCode= $("#projectManCode").val();
    var ajax1 = new $ax(Feng.ctxPath + "/lxProject/selectProjectManCode", function (data) {
        console.log(data);
        num=data;
    }, function (data) {
    });
    ajax1.set("projectManCode",projectManCode );
    ajax1.start();

    if (num>0){
        Feng.error("此项目编号已经使用");
        return false;
    }
 ...
 /*   if (ProjectMasterInfoDlg.projectAddress.length === 0) {
  ...
  /*  ProjectMasterInfoDlg.projectMasterInfoData.generalContractorName = $("#generalContractorName option:selected").html();*/
    ProjectMasterInfoDlg.projectMasterInfoData.generalContractorName = $("#citySel").val();
    //项目地址
    ...
    ProjectMasterInfoDlg.projectMasterInfoData.projectManCode = $("#projectManCode").val();
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/lxProject/add", function (data) {
        if (data==="ERROR") {
            Feng.error("添加失败!"+"已有此项目编号"+ "!");
        }else {
            $(".operation").hide();
            Feng.success("添加成功!");
...
ProjectMasterInfoDlg.editSubmit = function () {
    ...
    this.projectMasterInfoData.projectManCode = $("#projectManCode").val();
...
/*
    ...
    ztree.bindOnClick(function (event,data,c) {
        console.log(c)
        console.log(c.socialCreditNumber)
        $("#citySel").val(c.name);
        $("#contractorOrgCode").val(c.socialCreditNumber)
    });
    ztree.init();
    instance = ztree;
