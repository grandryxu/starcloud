...
/*    if($("#fixFlow").val()==1){
    ...
    ProjectMasterInfoDlg.projectMasterInfoData.projectId = $("#projectId").val();
    ProjectMasterInfoDlg.projectMasterInfoData.processId = $("#receiveSelect").val();
    ProjectMasterInfoDlg.projectMasterInfoData.flag = $("#receiveFlow").val();
    var list = $(".fd");
    var ids = "";
    for(var i=0;i<list.length;i++){
        ids =ids+ list[i].value+","
     }
    ProjectMasterInfoDlg.projectMasterInfoData.ids = ids;
    
    ProjectMasterInfoDlg.projectMasterInfoData.projectId = $("#projectId").val();
    ProjectMasterInfoDlg.projectMasterInfoData.fixProcessId = $("#fixSelect").val();
    ProjectMasterInfoDlg.projectMasterInfoData.fixFlag = $("#fixFlow").val();
    var list = $(".ffd");
    var fixIds = "";
    for(var i=0;i<list.length;i++){
    	fixIds =fixIds+ list[i].value+","
     }
    ProjectMasterInfoDlg.projectMasterInfoData.fixIds = fixIds;
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/lxProject/flow/add", function (data) {
        $(".operation").hide();
        Feng.success("添加成功!");
