...
LxOrder.history= function(){
    ...
    var index = layer.open({
        ...
        maxmin: true,
        skin: 'layer-no-title',
        content: Feng.ctxPath + '/lxOrder/history'
    });
...
LxOrder.initColumn = function () {
    return [
        ...
        {title: 'id', field: 'order.id', visible: false, align: 'center', valign: 'middle'},
         ...
         {title: '招标文件编号', field: 'business.tenderCode', visible: true, align: 'center', valign: 'middle'},
        {title: '招标文件名称', field: 'business.fileName', visible: true, align: 'center', valign: 'middle'},
        {title: '审批状态', field:'business.status' , visible: true, align: 'center', valign: 'middle',
...
LxOrder.bohui = function () {
    ...
    if(this.check()){
        var path = "../lxOrder/shenhe?orderId=" + LxOrder.seItem.order.id  + "&result=false&bussId=" + LxOrder.seItem.business.id;
...
LxOrder.openAddWorkerMaster = function () {
    ...
    if(this.check()){
        var path = "../lxOrder/shenhe?orderId=" + LxOrder.seItem.order.id  + "&result=true&bussId=" + LxOrder.seItem.business.id;
        $.post(path, { },function(data){
            if(data.code!=200){
                Feng.error(data.message);
            }else{
...
$(function () {
    ...
    var table = new BSTable(LxOrder.id, "/lxOrder/getAll", defaultColunms);
