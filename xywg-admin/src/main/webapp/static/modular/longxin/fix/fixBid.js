...
SubContractor.initColumn = function () {
    return [
        ...
        {title: '审核状态', field: 'flowStatus', visible: true, align: 'center', valign: 'middle',
        	...
        	}},
        {
            title: '操作', visible: true, align: 'center', valign: 'middle',width:"100px", formatter: function (val, row, index) {
            	if(row.status == 4){
            		return '<button  class="btn btn-sm btn-primary" style="background-color:green" onclick="SubContractor.openDetail('+row.id+ ',' + false+')" id="bid'+row.id+'"><strong><font color="white">定标中</font></strong></button>';
            	}else if(row.status == 3){
            		return '<button  class="btn btn-sm btn-primary" style="background-color:#4876FF" onclick="SubContractor.openDetail('+row.id+','+true+')" id="bid'+row.id+'"><strong><font color="white">定标</font></strong></button>';
            	}else {
            		return '<button  class="btn btn-sm btn-primary" style="background-color:green" onclick="SubContractor.openDetail('+row.id+','+false+')" id="bid'+row.id+'"><strong><font color="white">查看</font></strong></button>';
            	}
            	}
        }
 ...
 * 点击添加企业信息
 */
SubContractor.openDetail = function (e,flag) {
