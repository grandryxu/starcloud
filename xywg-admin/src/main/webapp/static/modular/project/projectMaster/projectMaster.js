...
SubContractor.openAddSubContractor = function (id) {
    var ajax = new $ax(Feng.ctxPath + "/lx/tendering/isBind?id=" + id, function (data) {
        if(data.code=="500"){
            ...
            var index = layer.open({
                ...
                maxmin: false,
                skin: "layer-detail",
                content: Feng.ctxPath + '/lx/tendering?id='+id
            });
...
ProjectMaster.initColumn = function () {
    return [
        ...
        {title: '合同查看', field:'psId' , visible: false, align: 'center', valign: 'middle',
            formatter: function (value, row, index) {
                if(row.fileNums === "0" ){
        ...
        {title: '管理招标', field: 'status', visible: true, align: 'center', valign: 'middle',
            formatter: function (value, row, index) {
                 return getZbHtml(row);
                   // return "<a size='2' color='blue' onclick='SubContractor.openAddSubContractor(\"" + row.id + "\")'>管理招标</a>";
 ...
 * 点击添加项目流程
 */
ProjectMaster.openAddProjectFlow = function () {
    if (this.check()) {
        var index = layer.open({
            type: 2,
            title: '添加项目流程',
            ...
            maxmin: false,
            skin: "layer-detail",
            content: Feng.ctxPath + '/lxProject/projectMaster_flow/' + ProjectMaster.seItem.id
        });
        layer.full(index);
        this.layerIndex = index;
    }
...
* 绑定mac页面
* */
ProjectMaster.openMac = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            maxmin: true,
            skin: "layer-detail",
            content: Feng.ctxPath + '/lxProject/projectMaster_mac/' + ProjectMaster.seItem.id
        });
        this.layerIndex = index;
    }
};
...
ProjectMaster.openProjectMasterDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            maxmin: true,
            skin: "layer-detail",
            content: Feng.ctxPath + '/projectMaster/projectMaster_update/' + ProjectMaster.seItem.id
        });
        this.layerIndex = index;
    }
};
...
$(function () {
    ...
    var table = new BSTable(ProjectMaster.id, "/projectMaster/list", defaultColunms);
