var trainInfoData = {
    ...
    detailList:null,  //被选中的工人数据
    workerNames:'' //被选中的工人名称
}
...
//项目值变动事件 重置选中工人
trainInfoData.projectChange = function () {
    trainInfoData.detailList = null;
    trainInfoData.workerNames = '';
    $("#workerNames").val('');
}
...
trainInfoData.submit = function () {
    ...
    savedData.detailList = trainInfoData.detailList;

    if(savedData.projectCode === ""){
        Feng.info("请选择项目");
        return;
    }

    if(savedData.detailList === null){
        Feng.info("请选择工人");
        return;
    }
...
//************************************************************** 选择工人 *****************************************************************
trainInfoData.chooseWorkers = function () {
    if($("#projectCode").val() === ''){
        Feng.info("请选择项目");
        return;
    }
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/projectTraining/projectTraining_workerList?projectCode='+$("#projectCode").val()
    });
    this.layerIndex = index;
}

function setCallBack(data) {
    trainInfoData.detailList = data.detailList;
    trainInfoData.workerNames = data.workerNames;
    $("#workerNames").val(data.workerNames.length>30?(data.workerNames.substring(0,30)+"..."):data.workerNames);
}
...
function getCheckedArray() {
    var data={};
    data.detailList = trainInfoData.detailList;
    data.workerNames = trainInfoData.workerNames;
    return data;
}