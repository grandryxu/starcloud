...
ProjectMaster.initColumn = function () {
    ...
    return [
        ...
        {title: '联系电话', field: 'phoneNumber', visible: true, align: 'center', valign: 'middle'},
 ...
 * 编辑班组
 */
ProjectMaster.openTeamMasterDetail = function () {
    if (this.check()) {
/*        if (hasOperationAuth()|| $("#page").val() === 'ZBXM') {
            if (ProjectMaster.seItem.isGeneralContractorOperation === 0) {
                Feng.error("您无权操作参建单位" + ProjectMaster.seItem.companyName + "的数据!");
                return;
            }
        }*/
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/teamMaster/teamMaster_update/' + ProjectMaster.seItem.id
        });
        this.layerIndex = index;
    }
};
...
ProjectMaster.openAddProjectMaster = function () {
    if ($("#projectCode").val()==null||$("#projectCode").val()==""){
        ...
        var index = layer.open({
            ...
            maxmin: true,
            skin: "layer-detail",
            content: Feng.ctxPath + '/teamMaster/addTeamView/'+$("#projectCode").val()
        });
        layer.full(index);
            this.layerIndex = index;
    }
...
ProjectMaster.openProjectMasterDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            maxmin: true,
            skin: "layer-detail",
            content: Feng.ctxPath + '/lxEvaluate/update/' + ProjectMaster.seItem.id
        });
        this.layerIndex = index;
    }
};
...
/*ProjectMaster.delete = function () {
    if (this.check()) {
        layer.confirm('确认删除？', {
            ...
            var ajax = new $ax(Feng.ctxPath + "/lxEvaluate/delete", function (data) {
                 Feng.success("删除成功!");
                 ProjectMaster.table.refresh();
            }, function (data) {
                Feng.error("删除失败!" + data.responseJSON.message + "!");
            });
            ajax.set("id", ProjectMaster.seItem.id);
            ajax.start();
            layer.close(index);
        });
    }
...
if (this.check()){
    layer.confirm('确认删除？',{
        ...
        ajax.set("teamMasterId", ProjectMaster.seItem.id);
        ajax.start();
        layer.close(index);
        }
    )
...
$(function () {
    ...
    var table = new BSTable(ProjectMaster.id, "/teamMaster/teamMasterListCount", defaultColunms);
