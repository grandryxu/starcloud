...
EmployeeMaster.mouldDownload = function () {
    window.location.href = "/static/excelMould/从业人员导入模板.xlsx";
};
...
EmployeeMaster.initColumn = function () {
    return [
        ...
        {title: '企业组织代码', field: 'organizationCode', visible: false, align: 'center', valign: 'middle'},
        ...
        {title: '离职时间', field: 'terminationDate', visible: true, align: 'center', valign: 'middle'},
        {title: '退休时间', field: 'retireDate', visible: true, align: 'center', valign: 'middle'},
    ];
};
...
EmployeeMaster.openEmployeeMasterDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            maxmin: true,
            skin: 'layer-no-title',
            content: Feng.ctxPath + '/employeeMaster/employeeMaster_update/' + EmployeeMaster.seItem.id + "/" + EmployeeMaster.seItem.organizationCode
        });
        this.layerIndex = index;
    }
};
...
EmployeeMaster.searchInfo = function (e) {
    var index = layer.open({
        ...
        maxmin: true,
        skin: 'layer-detail',
        content: Feng.ctxPath + '/employeeMaster/employeeMaster_view/' + e.id + "/" + e.organizationCode
    });
    this.layerIndex = index;
 ...
 * 入职
 */
EmployeeMaster.hire = function () {
    if (this.check()) {
        var id = this;
        layer.confirm('是否确认入职?', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            layer.closeAll();
            var ajax = new $ax(Feng.ctxPath + "/employeeMaster/hire", function (data) {
                Feng.success("入职成功!");
                EmployeeMaster.table.refresh();
            }, function (data) {
                Feng.error("入职失败!" + data.responseJSON.message + "!");
            });
            ajax.set("id", id.seItem.id);
            ajax.set("organizationCode", id.seItem.organizationCode);
            ajax.start();
        }, function () {
 ...
 * 离职
 */
EmployeeMaster.termination = function () {
    if (this.check()) {
        var id = this;
        layer.confirm('是否确认离职?', {
            ...
            layer.closeAll();
            var ajax = new $ax(Feng.ctxPath + "/employeeMaster/termination", function (data) {
                Feng.success("离职成功!");
                EmployeeMaster.table.refresh();
            }, function (data) {
 ...
 * 退休
 */
EmployeeMaster.retire = function () {
    if (this.check()) {
        var id = this;
        layer.confirm('是否确认退休?', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            layer.closeAll();
            var ajax = new $ax(Feng.ctxPath + "/employeeMaster/retire", function (data) {
                Feng.success("退休成功!");
                EmployeeMaster.table.refresh();
            }, function (data) {
                Feng.error( data.responseJSON.message + "!");
...
$(function () {
    ...
    var table = new BSTable(EmployeeMaster.id, "/employeeMa" +
        "ster/list", defaultColunms);
