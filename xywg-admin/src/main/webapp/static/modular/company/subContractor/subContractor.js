    ...
    return [
        ...
        {title: '企业名称', field: 'companyName', visible: true, align: 'center', valign: 'middle'},
        {
            ...
            formatter: function (value, row, index) {
                if(row.socialCreditNumber === row.bizRegisterCode){
                    return value;
                }else{
                    return null;
                }
...
SubContractor.updateContractorPassword=function(){
    if (this.checkOne()) {
        console.log(SubContractor.seItem.id);
        var ajax = new $ax(Feng.ctxPath + "/cooperationSubContractor/updateContractorPassword", function (data) {
                Feng.success("操作成功");
            }, function (data) {
                Feng.error("操作失败" + data.responseJSON.message + "!");
            }
        );
        ajax.set("id",SubContractor.seItem.id);
        ajax.start();
    }
}
...
SubContractor.openAddSubContractor = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: false,
        content: Feng.ctxPath + '/subContractor/subContractor_add_search'
...
SubContractor.openSubContractorDetail = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            maxmin: true,
            skin: 'layer-no-title',
            content: Feng.ctxPath + '/subContractor/subContractor_update/' + SubContractor.seItem.id
        });
...
SubContractor.checkOne = function () {
    var selected = $('#' + this.id).bootstrapTable('getSelections');
    if (selected.length == 0) {
        Feng.info("请先选中表格中的某一记录！");
        return false;
    } else if (selected.length > 1) {
        Feng.info("只能先选中表格中的某一记录！");
        return false;
    } else {
        SubContractor.seItem = selected[0];
        return true;
...
SubContractor.searchInfo = function (e) {
    var index = layer.open({
        ...
        maxmin: true,
        skin: 'layer-detail',
        content: Feng.ctxPath + '/subContractor/subContractor_view/' + e.id
    });
    this.layerIndex = index;
}
/**
