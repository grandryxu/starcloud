...
//    }
    ...
    // 是否审核：未审核
    this.subContractorInfoData.flowStatus = "0";
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/subContractor/addReg", function(data){
        if(data){
            Feng.success("注册成功，请等待审核！");
            // parent.getChildData(data,SubContractorInfoDlg.subContractorInfoData.companyName);
            // parent.layer.closeAll();
            // window.location.href=Feng.ctxPath+"/login?username="+SubContractorInfoDlg.subContractorInfoData.organizationCode + "&password=123456"

            $.post(Feng.ctxPath+"/loginReg",{"username":SubContractorInfoDlg.subContractorInfoData.organizationCode ,password:"123456"},function(data){
                if(data){
                    location.href = "/"
                }else{
                    location.href=  Feng.ctxPath + "/login"
                }
            })
