...
namespace Service
{
    public class WorkerService
    {
        ...
        private Dictionary<string, List<string>> trainClass = new Dictionary<string, List<string>>();//存储轨交分类统计表
        private List<string> workerTypeList = new List<string>();//存储所有已知轨交工种数据
        private Dictionary<string, int> workerTypeCountList = new Dictionary<string, int>();
        List<ClassCount> SubwayOutputList = new List<ClassCount>();//存储轨交统计数据模型

        public WorkerService()
        {
            trainClass.Add("第三方监测", "水保监测,监测人员".Split(',').ToList());
            trainClass.Add("第三方检测", "检测人员".Split(',').ToList());
            trainClass.Add("第三方测量", "测量人员,测量队队长,测量队长,测量员".Split(',').ToList());
            trainClass.Add("风险咨询", "巡查人员,队长,安全专监,质量专监,安全总监".Split(',').ToList());
            trainClass.Add("施工作业", "盾构操作手".Split(',').ToList());
            trainClass.Add("轨道公司", "工程部部长,员工,负责人,供电项目工程师,党委副书记,项目书记,办公室主任,办公室主任,主管,工程师,安质部部科员,安质部长,工程部科员,机电部长,财务负责人,计划部长,副经理,工程保障管理,总经理,总工程师,总工程师,综合办公司主任,综合办公司部员,项目工程师,档案工程师,试验检测工程师,征地征收工程师,市政前期工程师,干事,副总经理,技术主任,工程部长,工程副部长,物资部长,财务出纳,董事长".Split(',').ToList());
            trainClass.Add("施工管理", "管理人员,项目总工,施工单位管理人员,建设单位管理人员,工程副部长,安全员,协调员,土建主管,副队长,安质部部长,质量员,部门负责人,项目负责人,项目技术负责人,现场负责人,项目总监,总监代表,现场负责人,项目经理,项目副经理,工区主任,盾构经理,盾构副队长,材料员,核算员,资料员,计划员".Split(',').ToList());
            trainClass.Add("监理单位", "监理总监,监理单位管理人员,人防监理,人防施工,监理员,水保监测监理".Split(',').ToList());
            foreach (KeyValuePair<string, List<string>> kvp in trainClass)
            {
                workerTypeList.AddRange(kvp.Value);
            }
        }
...
#warning nResult = LedDll.LV_QuickAddSingleLineTextArea(hProgram, 1, 2, ref AreaRect, LedDll.ADDTYPE_STRING, "上海灵信视觉技术股份有限公司", ref FontProp, 4);//快速通过字符添加一个单行文本区域，函数见函数声明注示
        ...
        {
            string sql = string.Format("select count(id) as num from worker where groupname = '{0}' and checkinState = {1} and DATE(checkinTime) = '{2}';",
                groupanme,
                state,
                DateTime.Now.ToString("yyyy-MM-dd"));
            if (state > 1)//统计参与考勤人数
                sql = string.Format("select count(id) as num from worker where groupname = '{0}' and DATE(checkinTime) = '{1}';",
                groupanme,
                DateTime.Now.ToString("yyyy-MM-dd"));
            DataTable dt = SQLiteDBHelper.ExecuteDataTable(sql);
            DataRow row = null;
        ...
        {
            string sql = string.Format("select distinct job from worker where groupname = '{0}'", ConfigurationManager.AppSettings["prorealname"].ToString());
            DataTable dt = SQLiteDBHelper.ExecuteDataTable(sql);
            if (dt.Rows.Count <= 0)
                return;
            SubwayOutputList.Clear();
            workerTypeCountList.Clear();
            int AllCount = 0;
            //先统计项目下每个工种在场人数
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                string countsql = string.Format("select count(id) as count from worker where groupname = '{0}' and job = '{1}' and DATE(checkinTime) = '{2}' and checkinState = 1",
                    ConfigurationManager.AppSettings["prorealname"].ToString(),
                    dt.Rows[i]["job"].ToString(),
                    DateTime.Now.ToString("yyyy-MM-dd")
                    );
                ...
                if (dtt.Rows.Count >= 0)
                {
                    AllCount += int.Parse(dtt.Rows[0]["count"].ToString());
                    workerTypeCountList.Add(dt.Rows[i]["job"].ToString(), int.Parse(dtt.Rows[0]["count"].ToString()));
                    //完善“施工作业”类工种名称集合
                    if (!workerTypeList.Contains(dt.Rows[i]["job"].ToString()))
                        trainClass["施工作业"].Add(dt.Rows[i]["job"].ToString());
                }
            }
            ...
            {
                int countSource = 0;
                foreach (KeyValuePair<string, int> kv in workerTypeCountList)
                {
                    if (kvp.Value.Contains(kv.Key))
                        countSource += kv.Value;
                }
                SubwayOutputList.Add(new ClassCount
                {
                    classNo = kvp.Key,
                    count = countSource
                });
            ...
            Console.WriteLine(JsonConvert.SerializeObject(SubwayOutputList));
        }
        /// <summary>
        /// 考勤统计LED控制，轨交
        /// </summary>
        ...
        public void LedWorkGroupControl(LedInfo info, string identityId, int checktype)
        {
            try
            {
                ...
                foreach (ModuleInfo la in lla)
                {
                    ...
                    if (la.Area_type == 1 || la.Area_type == 5 || la.Area_type == 6)
                    {
                        ...
                        foreach (ClassCount cc in SubwayOutputList)
                        {
